...
@Builder //세터 사용하지 않고 빌더 패턴을 사용 아래 2개 어노테이션 필수
@Data
@AllArgsConstructor
@NoArgsConstructor
@Log4j2
public class PageRequestDTO {
    ...
    //페이징에 관련된 정보(page,size), 검색 종류(types->제목t, 내용c, 작성자w), 키워드(검색단어)

        @Builder.Default //빌더 패턴시작시 초기값이 들어감.
        private int page =1; //첫페이지

        @Builder.Default
        private int size = 10; //게시물 수

        private String type ; //t, c, w , tc, tw, t w c ... (다중검색용)
        ...
        public String getLink(){
            if(link == null) {
                ...
                link = builder.toString(); //최종결과물이 문자열로 변환되어 Link에 저장
            }//if문 종료
        ...
        public String[] getTypes() {
            ...
            //문자로 넘어온 값을 분할하여 배열에 꼽는다.
        }

        //테스트용 코드를 dto로 만들어 메서드 처리함!!
        public Pageable getPageable(String...props){ //String...props 배열이 몇개가 들어올지 모를떄
