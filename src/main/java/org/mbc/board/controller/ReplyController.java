...
@Log4j2 //import는 lombok껄로
public class ReplyController {
    //댓글용 컨트롤러로 Rest방식으로 처리
    //일반적으로 Controller와 혼용해서 사용 됨.



 /* p528에 CustomRestAdvice 용으로 변경
   @PostMapping(value = "/", consumes = MediaType.APPLICATION_JSON_VALUE) //post메서드로 json으로 처리용
    public ResponseEntity<Map<String,Long>> register(@RequestBody ReplyDTO replyDTO) {
      //리턴 타입이 응답용 객체에 key, value 로 전송하게 셋팅
        ...
        return ResponseEntity.ok(resultMap); //정상처리됨(200코드). 리턴타입은 프론트와 연결되지 않음
    ...
    public Map<String, Long> register(
            ...
            //검증용 코드        요청에 들어온 객체      리턴할 객체                          예외처리용
        ...
        resultMap.put("rno", 111L); //리턴 테스트용
        //              key value
        ...
        /*keypoint : ReplyDTO를 프론트에서 수집할 때 @Valid를 적용 (null이나 **등을 검증)
                      null이나 **가 들어오면 BindException을 throw함
